  ...
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full border border-[#404040] bg-[#171717] text-slate-400">
          <FiClock size={40} />
        </div>
    ...
    <div className="space-y-4 pb-12">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filteredHistory.map((doc, index) => (
          ...
          >
            <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl border border-[#525252] bg-[#1f1f1f] text-[#f59e0b]">
              <FiFileText size={22} />
            </div>
            ...
            <div className="mt-2 inline-flex rounded-full border border-[#525252] bg-[#171717] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-300">
              {doc.outputMode || "standard"}
            </div>

            <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-300">
              <div className="rounded-lg border border-[#404040] bg-[#171717] p-2">Words: {doc.stats?.wordCount || 0}</div>
              <div className="rounded-lg border border-[#404040] bg-[#171717] p-2">Risks: {doc.stats?.riskMentions || 0}</div>
            </div>
      ...
      {filteredHistory.length === 0 ? (
        <div className="rounded-2xl border border-[#404040] bg-[#121212] p-5 text-sm text-slate-400">
